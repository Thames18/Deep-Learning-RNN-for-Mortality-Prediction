...
plot_confusion_matrix(test_results, class_name)
		...
		for i, (input, target) in enumerate(data_loader):
			if isinstance(input, tuple):
				input = tuple([i.to(device) if type(i) == torch.Tensor else i for i in input])
				   # isinstance(i, torch.Tensor) else i for i in input])
				e1 = model(input)
				   #type(i) == torch.Tensor else i for i in input])
			else:
				input  = input.to(device)
			e1 = model(input)
			e2 = nn.Softmax(dim = 1)#nn.Softmax(dim=1)
			e3 = e2(e1)
			#pos = e3[:, 1]
			e3 = np.array(e3)
			e4 = list(e3[:, 1])
			probas.append(e4[0])
			#probas.extend(pos.detach().cpu().tolist)
	return probas
